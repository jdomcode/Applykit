    ...
    ? [
        {
          ...
          answer: "Sí. El resultado aparece en un campo editable para que puedas ajustarlo antes de copiarlo o guardarlo."
        },
        {
    ...
    : [
        {
          ...
          answer: "Yes. The result appears in an editable field so you can adjust it before copying or saving it."
        },
        {
...
export async function ToolDetail({ tool, locale }: Readonly<{ tool: ToolDefinition; locale: Locale }>) {
  ...
  await recordUsageEvent({
    eventType: "tool_viewed",
    locale,
    toolSlug: tool.slug,
    metadata: { source: "tool_detail" }
  });
  ...
  return (
    <Container className="py-8 sm:py-14">
      ...
      <section className="grid gap-8 rounded-[2rem] border border-slate-200 bg-white/80 p-6 shadow-sm sm:p-8 lg:grid-cols-[1fr_0.8fr] lg:items-start">
        <div>
        ...
        <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5 sm:p-6">
          ...
          <ul className="mt-4 space-y-3 text-sm leading-6 text-slate-600">
            ...
            <li>{locale === "es" ? "Generación real usando plantillas internas." : "Real generation using internal templates."}</li>
            <li>{locale === "es" ? "Página pública preparada para SEO." : "Public page prepared for SEO."}</li>
          </ul>
        </div>
      </section>

      <div className="mt-8"><ToolGenerator tool={tool} locale={locale} isAuthenticated={Boolean(user)} /></div>
