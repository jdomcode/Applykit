...
import { getDocumentsPath, getHomePath, getToolsPath } from "@/lib/i18n/navigation";
...
export async function Header({ locale }: Readonly<{ locale: Locale }>) {
  ...
  const { user, profile } = await getCurrentProfile(locale);
  const dashboardPath = `/${locale}/dashboard`;
  const adminPath = `/${locale}/admin/analytics`;
  ...
  const primaryLinks = (
    <>
      ...
      {user ? (
        <>
          <NavLink href={dashboardPath}>{dictionary.nav.dashboard}</NavLink>
          <NavLink href={getDocumentsPath(locale)}>{dictionary.nav.documents}</NavLink>
          {profile?.role === "admin" ? <NavLink href={adminPath}>Admin</NavLink> : null}
        </>
      ) : null}
    </>
  );
        ...
        <div className="hidden items-center gap-2 lg:flex">
          ...
          {user ? (
            <form action={signOutAction}>
              ...
              <button
                className="rounded-full bg-slate-100 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-200 hover:text-slate-950"
                type="submit"
              >
            ...
            <Link href={signInPath} className="rounded-full bg-slate-950 px-4 py-2 text-sm font-semibold text-white transition hover:bg-slate-800">
            ...
            <nav className="grid gap-1" aria-label={locale === "es" ? "Navegación móvil" : "Mobile navigation"}>
              ...
              {user ? (
                <form action={signOutAction}>
                  <input type="hidden" name="locale" value={locale} />
                  <button
                    className="w-full rounded-2xl px-3 py-3 text-left text-sm font-semibold text-slate-700 transition hover:bg-slate-100 hover:text-slate-950"
                    type="submit"
                  >
                    {dictionary.nav.signOut}
                  </button>
                ...
                <Link
                  href={signInPath}
                  className="rounded-2xl bg-slate-950 px-3 py-3 text-center text-sm font-semibold text-white transition hover:bg-slate-800"
                >
                  {dictionary.nav.signIn}
                </Link>
              )}
