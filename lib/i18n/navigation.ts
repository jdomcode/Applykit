...
export function getDocumentsPath(locale: Locale) {
  return locale === "es" ? "/es/documentos" : "/en/documents";
}

export function getDocumentDetailPath(locale: Locale, documentId: string) {
  return `${getDocumentsPath(locale)}/${documentId}`;
}
